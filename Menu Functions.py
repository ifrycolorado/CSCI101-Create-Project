from datetime import datetime
import csv

date = ""
type = ""
time = ""
piece = ""
goals = ""
intent = ""
section = ""
tempo = ""
attemptsTotal = ''
successfulAttempts = ''
unsuccessfulAttempts = ''
neutralAttempts = ''
reflection = ''

master_list = [date,type,time,piece,goals,intent,section,tempo,attemptsTotal,successfulAttempts,
                   unsuccessfulAttempts, neutralAttempts, reflection]

def main_menu():
    user_input = input("What would you like to do?> ")
    print("[1] Start Session\n[2] Analyze data")
    if user_input == "1":
        session_start()


def session_start():
    print("We are starting a session")
    date = datetime.now()
    type = "SessionStart"
    goals = input("What are your goals?> ")
    session_list = [date, type, time, piece, goals, intent, section, tempo, attemptsTotal, successfulAttempts,
                    unsuccessfulAttempts, neutralAttempts, reflection]
    with open('PT.csv', 'a') as PT_file:
        PT_writer = csv.writer(PT_file)
        PT_writer.writerow(master_list)


def activity_start():
    print('We are now starting a piece activity')
    date = datetime.now()
    type = "Activity1"
    piece = input("What piece are we practicing?> ")
    goals = input('What goals do you have for this piece?> ')
    intent = input('What measures do you want to run?> ')
    section = []
    tempo = []
    attemptsTotal = 0
    successfulAttempts = 0
    unsuccessfulAttempts = 0
    neutralAttempts = 0
    reflection = ""

    activity1_list = [date, type, time, piece, goals, intent, section, tempo, attemptsTotal, successfulAttempts,
                      unsuccessfulAttempts, neutralAttempts, reflection]
    candy_tracker()

def end_of_candy():
    print("Congrats on finishing that run?")
    print("What would you like to do next?")
    print("[1]")

def candy_tracker():
    print('We are now starting an individual run')

    date_run = datetime.now()
    type_run = "Activity2"
    time_run = ''
    piece_run = activity1_list[3]
    goals_run = ''
    intent_run = ''
    section_run = []
    section_run.append(input("What measure are we starting at?> "))
    section_run.append(input("What measure are we stopping at?> "))
    tempo_run = []
    tempo_run.append(input("What tempo is this run at?"))
    attemptsTotal_run = 0
    successfulAttempts_run = 0
    unsuccessfulAttempts_run = 0
    neutralAttempts_run = 0
    reflection_run = 0

    run_list = [date_run, type_run, time_run, piece_run,
                goals_run, intent_run, section_run, tempo_run,
                attemptsTotal_run, successfulAttempts_run,
                unsuccessfulAttempts_run, neutralAttempts_run, reflection_run]

    candy_length = int(input("Enter amount between 1 and 16 for candy> "))

    start_practice = time.time()

    candies_left_standard = "*" * candy_length
    candies_right_standard = ""
    candies_left = ""
    candies_right = ""

    format_string = '{left:<16}{right:>16}'

    user_input = 0
    successes = 0
    attempts = 0

    while user_input != "1":
        print("[1] Exit\n[2] Successful run\n[3] Negation\n[4] Unsuccessful run\n")
        user_input = input("Select choice: ")

        if user_input == "2":
            successes += 1
            run_list[9] += 1
        elif user_input == "3":
            run_list[11] += 1
        elif user_input == "4":
            successes -= 1
            run_list[10] += 1
        if successes <= 0:
            candies_left = candies_left_standard
            candies_right = ""
            successes = 0
        elif successes > candy_length:
            candies_left = candies_left = candies_left_standard[:(successes - candy_length)]
        else:
            candies_left = candies_left_standard[:-(successes - candy_length)]
        candies_right = candies_left_standard[:(candy_length - len(candies_left))]

        print(format_string.format(left="Left", right="Right"))
        print(format_string.format(left=candies_left, right=candies_right))
        print()

        attempts += 1
        run_list[8] += 1

        if successes == candy_length * 2:
            user_input = "1"

    stop_practice = time.time()

    time_run[2] = (stop_practice - start_practice)

    with open('PT.csv', 'a') as PT_file:
        PT_writer = csv.writer(PT_file)
        PT_writer.writerow(run_list)

    print("This is the end of the run")

    # FIXME kick back to main menu function

